...
const quests = [
  ...
  // --- TIER 2: NETWORK & RED TEAM ---
  {
    ...
    reward: 500,
    description:
      "Perform an aggressive Nmap scan on localhost and save the output to `recon.txt`.",
  ...
  {
    id: "q6",
    title: "Rogue Terminator",
    difficulty: "Medium",
    reward: 400,
    description:
      "A malicious process is running on PID 1337. Send a SIGKILL (-9) to terminate it.",
    objective: "kill -9 1337",
    check: (c, a) => c === "kill" && a.includes("-9") && a.includes("1337"),
  },
  {
    id: "q7",
    title: "The Firewall Architect",
    difficulty: "Ultimate",
    reward: 1000,
    description:
      "The host `10.0.0.99` is attacking! Use iptables to append (-A) a rule to the INPUT chain to DROP their source (-s) traffic.",
  ...
  },
  {
    ...
    reward: 600,
    description:
      "You've finished your Red Team operation. Overwrite your `~/.bash_history` file with nothing (an empty string) using echo.",
  ...
  // --- TIER 3: PERSISTENCE & PRIV ESC ---
  {
    ...
    reward: 400,
    description:
      "Check the active scheduled tasks for the current user to see if a backdoor is installed.",
    objective: "crontab -l",
    check: (c, a) => c === "crontab" && a.includes("-l"),
  },
  {
    id: "q10",
    title: "Data Exfiltrator",
    difficulty: "Ultimate",
    reward: 1200,
    description:
      "Archive the file `secret.txt` into a compressed tarball named `loot.tar.gz`.",
  ...
  {
    id: "q11",
    title: "Malware Profiler",
    difficulty: "Hard",
    reward: 800,
    description:
      "Generate a cryptographic SHA256 hash of the binary `/tmp/exploit.bin` and append it to `ir_log.txt`.",
    objective: "sha256sum /tmp/exploit.bin >> ir_log.txt",
    check: (c, a) =>
      c === "sha256sum" &&
      a.includes("/tmp/exploit.bin") &&
      a.includes(">>") &&
  ...
  },
  {
    ...
    reward: 1500,
    description:
      "Search the entire root filesystem (`/`) for files with the SUID permission bit set (`-perm -4000`).",
  ...
  // --- TIER 4: ADVANCED SYSADMIN ---
  {
    ...
    reward: 500,
    description:
      "Use usermod to append (-aG) the user 'hacker' to the 'wheel' group.",
  ...
  },
  {
    ...
    reward: 400,
    description:
      "Install the package 'nmap' using the apt package manager and auto-confirm (-y).",
    objective: "apt install nmap -y",
    check: (c, a) => c === "apt" && a.includes("install") && a.includes("nmap"),
  },
  {
    id: "q15",
    title: "The Silent Sleeper",
    difficulty: "Hard",
    reward: 600,
    description:
      "Run `sleep 600` in the background (&) completely immune to hangups using nohup.",
  ...
  },
  {
    ...
    reward: 600,
    description:
      "List the internal contents of `archive.tar.gz` without extracting it.",
  ...
  },
  {
    ...
    reward: 400,
    description:
      "Check the human-readable disk free space exclusively on the `/var/log` partition.",
  ...
  // --- TIER 5: OFFENSIVE WEAPONIZATION ---
  {
    ...
    reward: 1200,
    description:
      "Brute force SSH on 10.0.0.50 using user 'admin' and the 'rockyou.txt' password list.",
  ...
  },
  {
    ...
    reward: 800,
    description:
      "Run gobuster in 'dir' mode against 'http://10.0.0.50' using the wordlist 'words.txt'.",
  ...
  },
  {
    ...
    reward: 1500,
    description:
      "Use sqlmap against 'http://10.0.0.50/view.php?id=1' to completely dump the 'users' table from the 'admin_db' database.",
  ...
  {
    id: "q21",
    title: "Venomous Payload",
    difficulty: "Ultimate",
    reward: 1500,
    description:
      "Generate a linux elf reverse tcp shell using msfvenom targeting LHOST=10.0.0.99 and LPORT=4444.",
  ...
  },
  {
    ...
    reward: 1000,
    description:
      "Use hashcat to crack an MD5 hash (mode 0) in attack mode 0 against 'hash.txt' using 'rockyou.txt'.",
  ...
  {
    id: "q23",
    title: "Netcat Catcher",
    difficulty: "Medium",
    reward: 500,
    description:
      "Set up a listening, verbose, numeric netcat server on port 4444 to catch a reverse shell.",
  ...
  // --- TIER 6: BASH MAGIC & AUTOMATION ---
  {
    ...
    reward: 400,
    description:
      "Set and export an environment variable named TARGET pointing to '10.0.0.50'.",
    objective: "export TARGET=10.0.0.50",
    check: (c, a, o, raw) =>
      (c === "export" && a.some((x) => x.includes("TARGET"))) ||
      raw.includes("TARGET=10.0.0.50"),
  },
  {
    id: "q25",
    title: "Alias Architect",
    difficulty: "Medium",
    reward: 400,
    description: "Create an alias named 'll' that executes 'ls -la'.",
    objective: "alias ll='ls -la'",
    check: (c, a, o, raw) =>
      c === "alias" && raw.includes("ll=") && raw.includes("ls -la"),
  },
  {
    id: "q26",
    ...
    reward: 1200,
    description:
      "Write a for loop that pings the IPs 10.0.0.1 through 10.0.0.5 with 1 packet each.",
    objective: "for i in {1..5}; do ping -c 1 10.0.0.$i; done",
    check: (c, a, o, raw) =>
      c === "for" && raw.includes("ping") && raw.includes("{1..5}"),
  },
  {
    id: "q27",
    title: "The Awkward Parser",
    difficulty: "Hard",
    reward: 800,
    description:
      "Use awk to extract ONLY the first column (usernames) from `/etc/passwd`, using a colon ':' as the delimiter.",
  ...
  },
  {
    ...
    reward: 800,
    description:
      "Use sed to globally replace the word 'hate' with 'love' in a piped text stream.",
  ...
  {
    id: "q30",
    title: "Grandmaster Pipeline",
    difficulty: "Ultimate",
    reward: 2500,
    description:
      "Cut field 7 (shells) from `/etc/passwd` delimited by colons, sort them alphabetically, and count the unique occurrences.",
